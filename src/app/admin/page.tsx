...
export default async function AdminDashboard() {
  ...
  return (
    <section>
      ...
      <div className="admin-table"><h2>Últimos leads</h2>{recentLeads.map((lead) => <div className="admin-row" key={lead.id}><strong>{lead.name}</strong><span>{lead.phone}</span><span>{lead.status}</span></div>)}</div>
    </section>
  );
}
