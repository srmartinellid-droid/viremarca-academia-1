...
function gymJsonLd(data: Awaited<ReturnType<typeof getPublicData>>) {
  ...
  const hours = Object.entries(s?.openingHours ?? {}).flatMap(([day, slots]) =>
    (slots ?? []).map((slot) => ({
  ...
  const social = Object.values(s?.social ?? {}).filter(Boolean);
  return [
    {
      ...
      address: s?.address
        ? {
            "@type": "PostalAddress",
            streetAddress: [s.address.street, s.address.number].filter(Boolean).join(", "),
            ...
            <section className="section" key={section.key}>
              <div className="container">
                <Reveal>
                  ...
                  <h2>TREINO PARA VIDA REAL.</h2>
                </Reveal>
                <div className="grid grid-3">
                  {data.modalities.map((item) => (
                    <Link
                      href={"/modalidades/" + item.slug}
                      className="card modality-card"
                      key={item.id}
                      data-reveal="up"
                    >
                      <SmartImage
                      ...
                      <div className="chip-row">
                        ...
                        <span className="chip">{item.durationMin || 60} min</span>
                      </div>
                      <h3>{item.name}</h3>
            ...
            <section className="section" key={section.key}>
              <div className="container">
                ...
                <div className="grid grid-4">
                  {data.plans.map((item) => (
                    <article
                      className={"card plan-card " + (item.highlighted ? "featured" : "")}
                      key={item.id}
                    >
            ...
            <section className="section trial-section" id="trial" key={section.key}>
              <div className="container trial-content">
                <Reveal>
                  ...
                  <p>Preencha o formulário e receba o próximo passo no WhatsApp.</p>
                ...
                <div className="grid grid-3">
                  {data.instructors.map((item) => (
                    <article className="card" key={item.id}>
                      ...
                      />
                      <h3>{item.name}</h3>
                      <p className="muted">{item.specialties}</p>
                    </article>
                  ))}
                </div>
              </div>
            ...
            <section className="section" key={section.key}>
              <div className="container">
                <Reveal>
                  ...
                  <h2>UM AMBIENTE FEITO PARA TREINAR.</h2>
                </Reveal>
                <GalleryLightbox
                  items={data.gallery.map((g) => ({ ...g, hasImage: assetExists(g.imageUrl) }))}
                />
              </div>
            </section>
          );
            ...
            <section className="section alt" key={section.key}>
              <div className="container">
                <Reveal>
                  ...
                  <h2>TREINE TAMBÉM FORA DA ACADEMIA.</h2>
                </Reveal>
                <div className="grid grid-3">
                  {data.posts.map((post) => (
        ...
        if (section.key === "testimonials")
          return (
            <section className="section" key={section.key}>
              <div className="container">
                <Reveal>
                  <span className="eyebrow">QUEM TREINA</span>
                  <h2>EXPERIÊNCIAS DE DEMO.</h2>
                </Reveal>
                <div className="grid grid-3">
                  {data.testimonials.map((item) => (
                    <article className="card" key={item.id}>
                      {item.isDemo ? <span className="demo-badge">Demo</span> : null}
                      <p>“{item.quote}”</p>
          ...
          return (
            <section className="section" key={section.key}>
              <div className="container prose">
                ...
                <p className="status-open">● ABERTO AGORA · Horários demonstrativos</p>
...
function formatAddress(address: unknown) {
  ...
  return (
    [
      value.street && value.number ? value.street + ", " + value.number : value.street,
      value.district,
      value.city,
      value.state,
    ]
