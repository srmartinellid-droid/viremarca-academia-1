...
export function TrialForm({ modalities }: { modalities: Modality[] }) {
  ...
  return (
    <form className="trial-form" onSubmit={onSubmit}>
      ...
      <label>
        Modalidade
        <select name="modalityId" defaultValue="">
      ...
      <label className="check">
        <input type="checkbox" name="consent" required /> Aceito o tratamento dos dados conforme a{" "}
      ...
      <button className="btn" disabled={status === "sending"} type="submit">
        {status === "sending" ? "ENVIANDO..." : "QUERO MINHA AULA"}
      </button>
      {message ? (
        <p
